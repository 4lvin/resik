    ...
    return snapshot.data.data.isNotEmpty?SmartRefresher(
      ...
      child: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int i) {
            var tgl =
                DateFormat('dd/MM/yyyy').format(snapshot.data.data[i].tanggal);
            return Container(
                              ...
                              decoration: BoxDecoration(
                                  color: snapshot.data.data[i].status ==
                                      ...
                                      : snapshot.data.data[i].status ==
                                              "Sudah Diambil"
                                          ? Colors.green[400]
                                          : snapshot.data.data[i].status == "Proses"
                                              ? Colors.red[400]
                                              : snapshot.data.data[i].status ==
                                                      "Pending"
                                                  ? Colors.yellow[400]
                                  ...
                                  boxShadow: [
                                    BoxShadow(
                                        color: Colors.grey, offset: Offset(3.0, 3.0))
                                  ]),
                              child: Text(
                                "${snapshot.data.data[i].status}",
                                ...
                                child: Row(
                                  ...
                                  children: <Widget>[
                                    Padding(
                                      ...
                                      child: Text("${snapshot.data.data[i].tipe}"),
                                    ),
                                    Text("Rp. ${snapshot.data.data[i].harga}"),
                                  ],
                                ))
                          ],
                        ),
                      ),
                      snapshot.data.data[i].kode == "SP"?Align(
